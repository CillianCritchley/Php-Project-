...
//if(ISSET($_POST['submit'])) {
    ...
    $sql = "select firstname, surname, dateOfBirth, addressLine1, addressLine2, addTown, addCounty from Customer where customerID = " . $_POST['customerID'];
...
<br>
    <form name="unlock" action="" >
        <button type="button" onclick="unLock()" id="confirm"   name="submit" value="Confirm Customer "> Confirm </button>
        </form>
</div>
...
<input type="submit" id="createButton"  name="createButton" value="Confirm Customer ">
</div>
...
<script>
    ...
    function createDepAc()
    {
        <?php
        include 'db.inc.php';
     if(ISSET($_POST['createButton']) && $_REQUEST['customerID'] != 0 ) {
